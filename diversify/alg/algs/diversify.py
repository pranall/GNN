...
class Diversify(Algorithm):
    ...
    def extract_features(self, x):
        if self.use_gnn:
            ...
            for i in range(B):
                ...
                gnn_out = self.featurizer(sample.cuda(), edge_index, batch_size=1)
    ...
    def set_dlabel(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                # Flatten features if more than 2 dims, e.g. (B, C, T) -> (B, C*T)
                if feas.dim() > 2:
    ...
    def update(self, data, opt):
        ...
        print(f"[DEBUG] disc_input shape before flattening: {disc_input.shape}")

        # Flatten to [B, F] if it's 3D
        if disc_input.dim() > 2:
            disc_input = disc_input.view(disc_input.size(0), -1)
            print(f"[DEBUG] disc_input shape after flattening: {disc_input.shape}")
        ...
        print(f"[DEBUG] all_y shape: {all_y.shape}")
        print(f"[DEBUG] all_y dtype: {all_y.dtype}")
        print(f"[DEBUG] all_y sample: {all_y[:5]}")

        # Ensure labels are in index form
        if all_y.ndim > 1:
            all_y = torch.argmax(all_y, dim=1)

        # Squeeze predictions if they have shape [B, 1, C]

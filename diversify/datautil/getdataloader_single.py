...
# Global cache path for precomputed graphs
GRAPH_CACHE_PATH = os.path.join(os.path.dirname(__file__), 'precomputed_graphs.pt')
...
def get_dataloader(args, tr, val, tar):
    ...
    if isinstance(sample, (Data, tuple, dict)) and (
        isinstance(sample, Data) or 
        (isinstance(sample, tuple) and isinstance(sample[0], Data)) or
    ...
    return (
        DataLoader(tr, batch_size=args.batch_size, num_workers=args.N_WORKERS, 
                  shuffle=True, persistent_workers=args.N_WORKERS > 0),
        DataLoader(tr, batch_size=args.batch_size, num_workers=args.N_WORKERS, 
                  shuffle=False, persistent_workers=args.N_WORKERS > 0),
        DataLoader(val, batch_size=args.batch_size, num_workers=args.N_WORKERS, 
                  shuffle=False, persistent_workers=args.N_WORKERS > 0),
        DataLoader(tar, batch_size=args.batch_size, num_workers=args.N_WORKERS, 
                  shuffle=False, persistent_workers=args.N_WORKERS > 0),
    )

def get_act_dataloader(args):
    # Debug cache status
    print(f"\n🔍 Graph cache {'HIT ✅' if os.path.exists(GRAPH_CACHE_PATH) else 'MISS ❌'}")
    ...
    for i, person in enumerate(tmpp):
        ...
        if os.path.exists(GRAPH_CACHE_PATH) and not args.debug_mode:
            ...
            transform = None  # Skip transform since graphs are precomputed
            tdata = task.ActList(args, args.dataset, args.data_dir, tmpp, i, 
                               transform=transform, precomputed_graphs=graphs)
        else:
            ...
            if i == 0 and getattr(args, 'use_gnn', False):  # Save graphs from first domain
                torch.save(tdata.graphs, GRAPH_CACHE_PATH)
